...
try{
	...
	ResultSet rs=st.executeQuery("SELECT sum(total) FROM onpur.cart where  status='Payed' and cemail='"+email+"'");
	while(rs.next()){
		total=rs.getInt(1);
	}
...
<%
ResultSet rs1=st.executeQuery("SELECT * FROM ecommerceblock.cart  where  status='Payed' and cemail='"+email +"'");
while(rs1.next()){
	sno=sno+1;
